...
def user_can_use_node(user_data: Dict[str, Any], node: Dict[str, Any]) -> bool:
    """
    Определяет, доступна ли данная нода пользователю.

    Логика:
    - если у ноды нет tier → считаем её 'standard'
    - standard-ноды доступны всем
    - premium-ноды доступны только пользователям с plan='premium'
    - неизвестный tier считаем недоступным (защита от опечаток)
    """
...
def process_users(target_usernames: List[str]) -> List[Dict[str, Any]]:
    ...
    if default_pin:
        base_uri_params["pinSHA256"] = default_pin

    ip4 = hy2_env.get('IP4')
    ns_domain, ns_port, ns_subpath = (
        ns_env.get('HYSTERIA_DOMAIN'),
        ns_env.get('HYSTERIA_PORT'),
        ns_env.get('SUBPATH'),
    )
    ...
    for username in target_usernames:
        ...
        for node in nodes:
            if not user_can_use_node(user_data, node):
                continue
