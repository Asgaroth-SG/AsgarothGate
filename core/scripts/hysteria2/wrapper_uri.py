...
def process_users(target_usernames: List[str]) -> List[Dict[str, Any]]:
    ...
    for username in target_usernames:
        ...
        # IPv6 отключен, явно возвращаем None
        user_output["ipv6"] = None

        # Дополнительные ноды из nodes.json
        for node in nodes:
            ...
            # Тип ноды (Standard / Premium и т.д.)
            ...
            uri = generate_uri(
                ...
                ip=node_ip,
                port=node_port,
                uri_params=node_params,
                ip_version=ip_v,
                fragment_tag=tag,
            )

            # Добавляем typ ноды, чтобы фронт мог фильтровать Standard/Premium
            user_output["nodes"].append({
                "name": node_name,
                "uri": uri,
                "type": node_type,
            })

        # Ссылка нормальной подписки
