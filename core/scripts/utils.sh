...
check_core_version() {
    if systemctl is-active --quiet hysteria-server.service; then
        ...
        echo -e "Версия ядра Hysteria2: ${cyan}$HCVERSION${NC}"
    fi
}
...
check_version() {
    ...
    if version_greater_equal "$local_version" "$latest_version"; then
        echo -e "Версия панели: ${cyan}$local_version${NC}"
    else
        echo -e "Версия панели: ${cyan}$local_version${NC}"
        echo -e "Последняя версия: ${cyan}$latest_version${NC}"
        echo -e "${yellow}Список изменений версии $latest_version:${NC}"
...
load_hysteria2_env() {
    if [ -f "$CONFIG_ENV" ]; then
        ...
        echo "Ошибка: файл configs.env не найден. Используется SNI по умолчанию 'bts.com'."
        SNI="bts.com"
    fi
}
