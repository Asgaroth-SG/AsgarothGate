    ...
    if command -v caddy &> /dev/null; then
        ...
        if systemctl list-units --full -all | grep -q 'caddy.service'; then
            if systemctl is-active --quiet caddy.service; then
                echo -e "${yellow}Остановка и отключение службы caddy по умолчанию...${NC}"
...
[Unit]
Description=Служба подписок Hysteria Python
...
[Unit]
Description=Caddy для подписок Hysteria
After=network.target
...
start_service() {
    ...
    if systemctl is-active --quiet hysteria-normal-sub.service && systemctl is-active --quiet hysteria-caddy-normalsub.service; then
        echo -e "${green}Настройка службы Подписок завершена.${NC}"
        echo -e "${green}Базовый URL доступа: https://$HYSTERIA_DOMAIN:$HYSTERIA_PORT/$SUBPATH/sub/normal/{username}${NC}"
    else
        echo -e "${red}Настройка Подписок завершена, но одна или несколько служб не запустились.${NC}"
...
stop_service() {
    ...
    echo -e "${green}Службы Подписки остановлены и отключены. Конфигурационные файлы удалены.${NC}"
}
...
edit_subpath() {
    ...
    if [ ! -f "$NORMALSUB_ENV_FILE" ]; then
        echo -e "${red}Ошибка: Файл .env ($NORMALSUB_ENV_FILE) не найден. Пожалуйста, сначала выполните команду start.${NC}"
        exit 1
    fi

    if [[ ! "$new_path" =~ ^[a-zA-Z0-9]+$ ]]; then
        echo -e "${red}Ошибка: Новый подпуть должен содержать только буквенно-цифровые символы (a-z, A-Z, 0-9) и не может быть пустым.${NC}"
        exit 1
    fi

    if ! systemctl is-active --quiet hysteria-normal-sub.service || ! systemctl is-active --quiet hysteria-caddy-normalsub.service; then
        echo -e "${red}Ошибка: Одна или несколько служб не запущены. Пожалуйста, сначала запустите службы.${NC}"
        exit 1
    fi
    ...
    echo -e "${green}SUBPATH обновлен с '$old_subpath' на '$new_path' в $NORMALSUB_ENV_FILE.${NC}"
    ...
    if systemctl reload hysteria-caddy-normalsub.service 2>/dev/null; then
        echo -e "${green}Конфигурация Caddy успешно перезагружена.${NC}"
    else
        echo -e "${yellow}Перезагрузка не удалась, перезапуск службы Caddy...${NC}"
        systemctl restart hysteria-caddy-normalsub.service
    fi

    if systemctl is-active --quiet hysteria-normal-sub.service && systemctl is-active --quiet hysteria-caddy-normalsub.service; then
        echo -e "${green}Службы успешно обновлены.${NC}"
        echo -e "${green}Новый базовый URL доступа: https://$HYSTERIA_DOMAIN:$HYSTERIA_PORT/$new_path/sub/normal/{username}${NC}"
        echo -e "${cyan}Старый подпуть '$old_subpath' больше не доступен.${NC}"
    else
        echo -e "${red}Ошибка: Не удалось перезапустить/перезагрузить одну или несколько служб. Проверьте логи.${NC}"
        systemctl status hysteria-normal-sub.service --no-pager
        systemctl status hysteria-caddy-normalsub.service --no-pager
    fi
}
...
case "$1" in
    start)
        if [ -z "$2" ] || [ -z "$3" ]; then
            echo -e "${red}Использование: $0 start <EXTERNAL_DOMAIN> <EXTERNAL_PORT>${NC}"
            exit 1
        fi
        start_service "$2" "$3"
        ;;
    ...
    edit_subpath)
        if [ -z "$2" ]; then
            echo -e "${red}Использование: $0 edit_subpath <NEW_SUBPATH>${NC}"
            exit 1
        fi
        edit_subpath "$2"
        ;;
    *)
        echo -e "${red}Использование: $0 {start <EXTERNAL_DOMAIN> <EXTERNAL_PORT> | stop | edit_subpath <NEW_SUBPATH>}${NC}"
        exit 1
        ;;
esac